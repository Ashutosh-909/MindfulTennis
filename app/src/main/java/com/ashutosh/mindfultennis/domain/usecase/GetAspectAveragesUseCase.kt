...
/**
 * Computes the average self-rating for each of the 8 aspects
 * across sessions within a date range, optionally filtered by opponent IDs.
 */
class GetAspectAveragesUseCase @Inject constructor(
    ...
    operator fun invoke(
        ...
        return sessionRepository.observeSessionsInRange(userId, fromMs, toMs).map { sessions ->
            val filtered = filterByOpponents(sessions, opponentIds)
                .filter { !it.isActive }
            computeAverages(filtered)
        }
    }

    private suspend fun computeAverages(sessions: List<Session>): Map<Aspect, Float> {
        if (sessions.isEmpty()) return emptyMap()

        val sessionIds = sessions.map { it.id }
        val allRatings = sessionRepository.getSelfRatingsForSessions(sessionIds)
